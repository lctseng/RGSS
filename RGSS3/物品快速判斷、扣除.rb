...
$lctseng_scripts[:item_cost] = "1.00"
...
class Game_Interpreter
  ...
  def enter_task_require(type , id , number)
    @check_task_item_array.push([type,id,number])
  end
  #--------------------------------------------------------------------------
  # ● 執行判斷並回傳結果至呼叫端
  #       cost : 是否自動扣除
  ...
  def run_task_judge(cost = false)
    for i in 0...@check_task_item_array.size
      type = @check_task_item_array[i][0]
      id = @check_task_item_array[i][1]
      n = @check_task_item_array[i][2]
      case type
      when 0
        if p_Gold < n
          end_judgement
